...
class TestCollectionAfter:
    ...
    def test_after_with_mixed_types(self):
        ...
        assert mixed.after(lambda x: isinstance(x, list)) is None  # List is last element
    ...
    def test_after_with_none_values(self):
        ...
        assert collection.after(lambda x: x is None and collection.all().index(x) == 4) is None
