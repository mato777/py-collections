import pytest
from src.py_collections.collection import Collection, ItemNotFoundException
...
class TestCollectionFirstOrRaise:
    ...
    def test_first_or_raise_empty_collection(self):
        ...
        with pytest.raises(ItemNotFoundException, match="Cannot get first element from empty collection"):
    ...
    def test_first_or_raise_no_match(self):
        ...
        with pytest.raises(ItemNotFoundException, match="No element satisfies the predicate"):
    ...
    def test_first_or_raise_consistency_with_first(self):
        ...
        # Test with predicate
        assert collection.first_or_raise(lambda x: x % 2 == 0) == collection.first(lambda x: x % 2 == 0)
        assert collection.first_or_raise(lambda x: x > 3) == collection.first(lambda x: x > 3)
