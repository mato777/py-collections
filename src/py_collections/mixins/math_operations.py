...
class MathOperationsMixin[T]:
    ...
    def average( # noqa: PLR0912
        self, key_or_callback: str | Callable[[T], int | float] | None = None
    ) -> float:
        """
        Calculate the average of items in the collection.

        Args:
            key_or_callback: Optional key or callback function.
                - If None: averages all numeric values in the collection
                - If str: averages the values of the specified key/attribute from each item
                - If callable: averages the results returned by the callback for each item

        Returns:
            The average of the values.

        Raises:
            ValueError: If no numeric values are found when averaging without arguments.
            AttributeError: If the specified key doesn't exist on items.
            TypeError: If the callback doesn't return a numeric value.
        ...
        Examples:
            ...
            >>> items.average(lambda x: x["price"] * 1.1)  # With 10% tax
            22.0
        """
        if key_or_callback is None:
            numeric_items = [
            ...
            if not numeric_items:
                raise ValueError("No numeric values found in collection to average")
            ...
            for item in self._items:
                ...
                values.append(value)
            if not values:
                raise ValueError("No values found for key to average")
            ...
            if not results:
                raise ValueError("No results found from callback to average")
